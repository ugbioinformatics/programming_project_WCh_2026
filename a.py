...
#def load_xtb_xyz(path,obj_name): #chyba mozna bez problemu usunąć bo nie jest nigdy uzywane !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def play_vib(xyz,mode,elem, out_file="vib_xyz.xyz"): # load up the frames for a vibration of primitive cell
        import numpy as np
        import math
...
#        model_o = cmd.get_model('geom', 1) # input starting geometry
        ...
        #xyz_arr = np.array(np.float_(xyz)) <-nie dziala bo jest stary ale jak nie dzial to moze to cofnąć????????????????????????????????????????????????
...
#xyz,elem=load_xtb_xyz('g98.out',"geom") stare mozna usunąć !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
